...
from kyan import bencode, models, utils
from kyan.extensions import config
from kyan.models import User
...
def register_email_blacklist_validator(form, field):
    ...
    for item in email_blacklist:
        if isinstance(item, re.Pattern):
            ...
            raise Exception(
                "Unexpected email validator type {!r} ({!r})".format(type(item), item)
            )
    return True
...
def register_email_server_validator(form, field):
    ...
    try:
        ...
        app.logger.error(
            "Unable to query MX records for email: %s - ignoring", email, exc_info=False
        )
        return True
        ...
        try:
            ...
            for a_record in a_records:
                ...
                if a_record.address in server_blacklist:
                    app.logger.warning(
                        "Rejected email %s due to blacklisted mailserver (%s, %s)",
                        email,
                        a_record.address,
                        mx_record.exchange,
                    )
            ...
            app.logger.warning(
                "Failed to query A records for mailserver: %s (%s) - ignoring",
                mx_record.exchange,
                email,
                exc_info=False,
            )
...
class PasswordResetRequestForm(FlaskForm):
    email = StringField(
        "Email address", [Email(), DataRequired(), Length(min=5, max=128)]
    )
    ...
    comment = TextAreaField(
        ...
        [
            Length(
                min=3,
                max=2048,
                message="Comment must be at least %(min)d characters "
...
class InlineButtonWidget(object):
    ...
    def __call__(self, field, label=None, **kwargs):
        ...
        return str(
            Markup("<button %s>" % self.html_params(name=field.name, **kwargs) + label)
        )
...
class EditForm(FlaskForm):
    display_name = StringField(
        ...
        [
            Length(
                min=3,
                max=255,
                message="Torrent display name must be at least %(min)d characters "
                "long and %(max)d at most.",
            )
        ],
    )
    ...
    information = StringField(
        ...
        [
            Length(
                max=255, message="Information must be at most %(max)d characters long."
            )
        ],
    )
...
class UploadForm(FlaskForm):
    ...
    def validate_torrent_file(form, field):
        ...
        if torrent_dict["info"].get("private") == 1:
            if torrent_dict["announce"].decode("utf-8") != site_tracker:
                raise ValidationError(
                    "Private torrent: please set {} as the main tracker".format(
                        site_tracker
                    )
                )
        ...
        if existing_torrent and not existing_torrent.deleted:
            raise ValidationError(
                "This torrent already exists (#{})".format(existing_torrent.id)
            )
...
class ReportActionForm(FlaskForm):
    action = SelectField(
        choices=[("close", "Close"), ("hide", "Hide"), ("delete", "Delete")]
    )
    torrent = HiddenField()
    report = HiddenField()
...
class TrustedForm(FlaskForm):
    why_give_trusted = TextAreaField(
    ...
    why_want_trusted = TextAreaField(
...
class TrustedReviewForm(FlaskForm):
    comment = TextAreaField(
        "Comment", [Length(min=8, max=4000, message="Please provide a comment")]
    )
    recommendation = SelectField(
        choices=[("abstain", "Abstain"), ("reject", "Reject"), ("accept", "Accept")]
    )
...
def _validate_trackers(torrent_dict, tracker_to_check_for=None):
    ...
    tracker_found = (
        tracker_to_check_for
        and (announce_string.lower() == tracker_to_check_for.lower())
        or False
    )
    ...
    if test_decode:
        try:
            ...
            raise AssertionError(
                name + " could not be decoded from " + repr(test_decode)
            )


def _validate_number(
    value, name="value", check_positive=False, check_positive_or_zero=False
):
