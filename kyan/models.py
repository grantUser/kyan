...
from kyan.extensions import config, db
from kyan.torrents import create_magnet
...
class FlagProperty(object):
    ...
    def __set__(self, instance, value):
        new_flags = (self._get_flags(instance) & ~self._flag) | (
            bool(value) and self._flag
        )
...
class TorrentBase(object):
    ...
    def main_category_id(cls):
        fk = db.ForeignKey("main_categories.id")
    ...
    @declarative.declared_attr
    def user(cls):
        return db.relationship("User", uselist=False, back_populates="torrents")
    ...
    def trackers(cls):
        return db.relationship(
            ...
            uselist=True,
            cascade="all, delete-orphan",
            order_by="TorrentTrackers.order",
        )

    @declarative.declared_attr
    def comments(cls):
        return db.relationship("Comment", uselist=True, cascade="all, delete-orphan")
    ...
    def update_comment_count_db(cls, torrent_id):
        cls.query.filter_by(id=torrent_id).update(
            {
    ...
    def information_as_link(self):
        ...
        irc_match = re.match(
            r"^#([a-zA-Z0-9-_]+)@([a-zA-Z0-9-_.:]+)$", self.information
        )
        if irc_match:
        ...
        if url_match:
            ...
            if not any(c in url for c in invalid_url_characters):
                return (
                    ...
                    'href="{0}">{1}</a>'.format(url, Markup(unquote_url(url)))
                )

        return Markup(self.information)
...
class TorrentFilelistBase(object):
    __tablename__ = "torrents_filelist"

    __table_args__ = {"mysql_row_format": "COMPRESSED"}

    @declarative.declared_attr
    def torrent_id(cls):
        fk = db.ForeignKey("torrents.id", ondelete="CASCADE")
    ...
    @declarative.declared_attr
    def torrent(cls):
        return db.relationship("Torrent", uselist=False, back_populates="filelist")
...
class StatisticBase(object):
    ...
    @declarative.declared_attr
    def torrent(cls):
        return db.relationship("Torrent", uselist=False, back_populates="stats")
...
class Trackers(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    uri = db.Column(
        db.String(length=255, collation=COL_UTF8_GENERAL_CI),
        nullable=False,
        unique=True,
    )
...
class TorrentTrackersBase(object):
...
class MainCategoryBase(object):
    ...
    def sub_categories(cls):
        return db.relationship("SubCategory", back_populates="main_category")

    @declarative.declared_attr
    def torrents(cls):
        return db.relationship("Torrent", back_populates="main_category")
...
class CommentBase(object):
    ...
    @declarative.declared_attr
    def torrent(cls):
        return db.relationship("Torrent", uselist=False, back_populates="comments")
...
class User(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(
        db.String(length=32, collation=COL_ASCII_GENERAL_CI),
        unique=True,
        nullable=False,
    )
    email = db.Column(
        EmailType(length=255, collation=COL_ASCII_GENERAL_CI),
        unique=True,
        nullable=True,
    )
    password_hash = db.Column(
        PasswordType(max_length=255, schemes=["argon2"]), nullable=False
    )
    ...
    last_login_date = db.Column(
        db.DateTime(timezone=False), default=None, nullable=True
    )
    ...
    def satisfies_trusted_reqs(self):
        num_total = 0
        downloads_total = 0
        for ts_flavor, t_flavor in ((Statistic, Torrent),):
            uploads = (
                db.session.query(func.count(t_flavor.id))
                .filter(t_flavor.user == self)
                .filter(t_flavor.flags.op("&")(int(TorrentFlags.REMAKE)).is_(False))
                .scalar()
            )
            dls = (
                db.session.query(func.sum(ts_flavor.download_count))
                .join(t_flavor)
                .filter(t_flavor.user == self)
                .filter(t_flavor.flags.op("&")(int(TorrentFlags.REMAKE)).is_(False))
                .scalar()
...
class ReportBase(object):
    ...
    @declarative.declared_attr
    def torrent(cls):
        return db.relationship("Torrent", uselist=False, lazy="joined")
...
class Ban(db.Model):
    ...
    admin = db.relationship(
        "User", uselist=False, lazy="joined", foreign_keys=[admin_id]
    )
        ...
        if user_id:
            if user_ip:
                return cls.query.filter(
                    (cls.user_id == user_id) | (cls.user_ip == user_ip)
                )
...
class TrackerApiBase(object):
    __tablename__ = "trackerapi"
...
class RangeBan(db.Model):
    ...
    masked_cidr = db.Column(db.BigInteger, nullable=False, index=True)
    ...
    def cidr_string(self, s):
        ...
        if network.version == 4:
            subnet_b = int(network.network_address)
            self.mask = int(network.netmask)
        elif network.version == 6:
            subnet_b = int(network.network_address)
            self.mask = int(network.netmask)
        else:
            raise ValueError("Invalid IP version in CIDR string")
        self.masked_cidr = subnet_b & self.mask
        self._cidr_string = s
    ...
    def is_rangebanned(cls, ip):
        ...
        elif isinstance(ip, bytes) and len(ip) == 16:
            ip_int = int.from_bytes(ip, "big")
        else:
            raise ValueError("Invalid IP address format")

        q = cls.query.filter(cls.mask.op("&")(ip_int) == cls.masked_cidr, cls.enabled)
        return q.count() > 0
...
class TrustedApplication(db.Model):
    ...
    status = db.Column(
        ChoiceType(TrustedApplicationStatus, impl=db.Integer()),
        nullable=False,
        default=TrustedApplicationStatus.NEW,
    )
    reviews = db.relationship("TrustedReview", backref="trusted_applications")
    submitter = db.relationship(
        "User", uselist=False, lazy="joined", foreign_keys=[submitter_id]
    )
...
class TrustedReview(db.Model):
    ...
    app_id = db.Column(
        db.Integer, db.ForeignKey("trusted_applications.id"), nullable=False
    )
    created_time = db.Column(db.DateTime(timezone=False), default=datetime.utcnow)
    comment = db.Column(db.String(length=4000), nullable=False)
    recommendation = db.Column(
        ChoiceType(TrustedRecommendation, impl=db.Integer()), nullable=False
    )
    reviewer = db.relationship(
        "User", uselist=False, lazy="joined", foreign_keys=[reviewer_id]
    )
    application = db.relationship(
        ...
        lazy="joined",
        foreign_keys=[app_id],
        overlaps="reviews,trusted_applications",
    )
...
# Torrent
class Torrent(TorrentBase, db.Model):
    ...
...
# Statistic
class Statistic(StatisticBase, db.Model):
    ...


# TorrentTrackers
class TorrentTrackers(TorrentTrackersBase, db.Model):
    ...


# MainCategory
class MainCategory(MainCategoryBase, db.Model):
    pass


# SubCategory
class SubCategory(SubCategoryBase, db.Model):
    ...


# Comment
class Comment(CommentBase, db.Model):
    ...


# AdminLog
class AdminLog(AdminLogBase, db.Model):
    ...


# Report
class Report(ReportBase, db.Model):
    ...


# TrackerApi
class TrackerApi(TrackerApiBase, db.Model):
    ...
