...
from kyan import email, forms, models
from kyan.extensions import db, limiter
from kyan.utils import sha1_hash
from kyan.views.users import (get_activation_link, get_password_reset_link,
                              get_serializer)
...
def login():
    ...
    if flask.request.method == "POST" and form.validate():
        ...
        if user.is_banned:
            ...
            ban_str = (
                "<strong>Login failed!</strong> You are banned with the "
                'reason "{0}" If you believe that this is a mistake, contact '
                "a moderator on IRC.".format(ban_reason)
            )
            flask.flash(str(Markup(ban_str)), "danger")
            return flask.redirect(flask.url_for("account.login"))

        if user.status != models.UserStatusType.ACTIVE:
            flask.flash(
                "<strong>Login failed!</strong> Account is not activated.",
                "danger",
            )
            return flask.redirect(flask.url_for("account.login"))
                ...
                str(
                    Markup(
                        ...
                        "activate your account <a href=\"{}\">'{}'</a>.".format(
                            ...
                            flask.url_for("users.view_user", user_name=user.username),
                            user.username,
                        )
                    )
                ),
...
def password_reset(payload=None):
    ...
    if payload is None:
        ...
        sha1_password_hash_hash = binascii.hexlify(
            sha1_hash(user.password_hash.hash)
        ).decode()
        ...
        if flask.request.method == "POST" and form.validate():
            ...
            flask.flash("Your password was reset. Log in now.", "info")
            return flask.redirect(flask.url_for("account.login"))
    ...
    if flask.request.method == "POST":
        if form.authorized_submit and form.validate():
            ...
            if new_email:
                ...
                flask.flash(
                    str(Markup("<strong>Email successfully changed!</strong>")),
                    "success",
                )
            ...
            if new_password:
                ...
                flask.flash(
                    str(Markup("<strong>Password successfully changed!</strong>")),
                    "success",
                )
            db.session.add(user)
            db.session.commit()
...
def request_trusted():
    ...
    if (
        models.TrustedApplication.query.filter(
            models.TrustedApplication.submitter_id == flask.g.user.id
        )
    ...
    return flask.render_template(
        "trusted_form.html", trusted_form=trusted_form, deny_reasons=deny_reasons
    )
