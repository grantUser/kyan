...
def view_adminbans():
    ...
    return flask.render_template("admin_bans.html", bans=bans, form=form)
...
def view_reports():
    ...
    return flask.render_template(
        "reports.html", reports=reports, report_action=report_action
    )
...
def view_trusted(list_filter=None):
    ...
    if list_filter == "closed":
        ...
        if list_filter == "new":
            ...
            flask.abort(404)
    apps = q.order_by(models.TrustedApplication.created_time.desc()).paginate(
        page=page, per_page=20
    )

    return flask.render_template(
        "admin_trusted.html", apps=apps, list_filter=list_filter
    )


@bp.route(
    "/trusted/application/<int:app_id>",
    endpoint="trusted_application",
    methods=["GET", "POST"],
)
def view_trusted_application(app_id):
    ...
    if flask.request.method == "POST":
        do_decide = decision_form and (
            decision_form.accept.data or decision_form.reject.data
        )
        if do_decide and decision_form.validate():
            ...
            if decision_form.accept.data:
                ...
                flask.flash(
                    str(Markup("Application has been <b>accepted</b>.")), "success"
                )
            elif decision_form.reject.data:
                app.status = models.TrustedApplicationStatus.REJECTED
                flask.flash("Application has been <b>rejected</b>.", "success")
            _send_trusted_decision_email(app.submitter, bool(decision_form.accept.data))
            db.session.commit()
            return flask.redirect(
                flask.url_for("admin.trusted_application", app_id=app_id)
            )
            ...
            tr.recommendation = getattr(
                models.TrustedRecommendation, review_form.recommendation.data.upper()
            )
